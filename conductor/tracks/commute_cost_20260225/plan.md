...
- [ ] Task: Update `TflClient` to extract fare information (peak/off-peak) from the Journey Planner API response (TDD - Red Phase)
- [ ] Task: Implement fare extraction logic in `TflClient._parse_journey_response` or similar (TDD - Green Phase)
- [ ] Task: Update the `TflClient` caching mechanism to include a `fares` field in the stored JSON (TDD - Green/Refactor)
